            ...
            <div className="flex justify-center flex-col items-center gap-4">
              ...
              <div className="flex justify-between w-full items-center">
                <div className="flex flex-col">
                  <div className="text-2xl text-[#725868] font-bold">Main Menu</div>
                  <div className="text-sm text-[#4D4449] font-semibold ">Quick Actions</div>
                </div>
                <button onClick={()=>setOpen(false)} className="bg-[#F4D2E5] p-2 rounded-full">
              ...
              <div className="flex py-4 justify-start  items-center gap-16 text-start p-2 flex-wrap">
                <Link href={"/dashboard"} className={`flex gap-4 flex-col items-center justify-center `}>
                  <span className={`rounded-xl text-[#725868] p-2 ${pathName === "/dashboard" ? "bg-[#F4D2EF]" : "bg-[#FDF2F8]"}`}>
                    <LayoutGrid size={32} strokeWidth={2} />
                  </span>
                  <span className="text-sm font-bold text-[#4D4449]">Dashboard</span>
                </Link>
                <Link href={"/dashboard/addexpense"} className="flex gap-2 flex-col items-center justify-center">
                    <span className={`rounded-xl text-[#725868] p-2 ${pathName === '/dashboard/addexpense' ? "bg-[#F4D2EF]" : "bg-[#FDF2F8]"}`}>
                    <Plus size={32} strokeWidth={2}  />
                  </span>
                  <span className="text-sm font-bold text-[#4D4449]">Add Expense</span>
                </Link>
                <Link href={"/dashboard/addexpense"} className={ `flex gap-2 flex-col items-center justify-center`}>
                  <span className={`rounded-xl text-[#725868] p-2 ${pathName === "/dashboard/addexpense" ? "bg-[#F4D2EF]" : "bg-[#FDF2F8]"} `}>
                    <Banknote size={32} strokeWidth={2} />
                  </span>
                  <span className="text-sm font-bold text-[#4D4449]">Expenses</span>
                </Link>
                <Link href={"/dashboard/expenses"} className="flex gap-2 flex-col items-center justify-center">
                  <span className={`rounded-xl ${pathName === '/dashboard/expense' ? "bg-[#F4D2E5] " : "bg-[#FDF2F8]"} text-[#725868]  p-2 `}>
                    <WalletIcon size={32} strokeWidth={2} />
                  </span>
                  <span className={`text-sm font-bold text-[#4D4449]`}>Budgets</span>
                </Link>
